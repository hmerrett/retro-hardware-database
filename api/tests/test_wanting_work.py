...
what needs doing, and /projects gathers them under "Wanting work" -- the private
half of a page whose other half, the named projects, is public. See
test_projects.py for that half; this file is about the flag.
...
def listed(client):
    """The asset tags in the "Wanting work" queue, in the order it draws them.

    The pattern matches a card's own header link -- tag as both href and text --
    which is a shape nothing in the projects half of the page has, so it still
    reads only the queue now the two share a page."""
...
class TestItIsPrivate:
    ...
    def test_a_visitor_may_read_the_page_but_not_the_queue(self, client, computer,
                                                            monkeypatch):
        """The page stopped being private when the named projects moved onto it --
        a project is a piece of work worth reading about. The queue did not: it is
        what is wrong with things, in the owner's own words, and it is drawn for a
        signed-in reader only."""
        c = computer(model="A500")
        flag(client, c["asset_id"], "recap, one leg already green")
        visitor(monkeypatch)
        page = client.get("/projects")
        assert page.status_code == 200
        assert "Wanting work" not in page.text
        assert "recap, one leg already green" not in page.text
        assert c["asset_id"] not in page.text

    def test_whoever_is_logged_in_gets_the_queue(self, client, computer):
        c = computer(model="A500")
        flag(client, c["asset_id"], "recap, one leg already green")
        page = client.get("/projects").text
        assert "Wanting work" in page
    ...
    def test_the_page_is_offered_in_the_menu_to_everybody(self, client,
                                                          monkeypatch):
        """It was behind the login and the link with it. Now the page is public and
        the link is too -- what a visitor arriving on it does not get is the queue,
        which is the thing that was ever private."""
