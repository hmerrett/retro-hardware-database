...
def test_a_photograph_from_a_phone_is_accepted(client):
    r = client.post("/computers/new", data={"model": "Phoneshot"},
                    files={"photos": ("IMG_4021.JPG", phone_photo(),
                                      "image/jpeg")},
                    follow_redirects=False)
...
def test_a_phone_photograph_uploaded_to_an_item_is_accepted(client, computer):
    ...
    r = client.post(f"/computers/{aid}/photo",
                    files={"photos": ("IMG_4022.JPG", phone_photo(),
                                      "image/jpeg")},
                    follow_redirects=False)
    assert r.status_code in (200, 303), r.text
...
def test_a_phone_photograph_can_still_be_rotated(client, computer):
    ...
    r = client.post(f"/computers/{aid}/photo-rotate",
                    data={"image": rel, "dir": "cw"}, follow_redirects=False)
    assert r.status_code in (200, 303), r.text
