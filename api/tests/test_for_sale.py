...
class TestTickingAnItem:
    ...
    def test_the_tick_sets_it_and_unticking_takes_it_back(self, client, db, part,
                                                          computer, kind, model):
    ...
    def test_ticking_something_that_is_not_there_is_a_404(self, client):
        assert client.post("/parts/RH-9999/for-sale",
                           data={"for_sale": "1"}).status_code == 404
...
class TestNobodyElseSeesIt:
    ...
    def test_a_visitor_cannot_tick_one(self, client, part, monkeypatch):
        aid = part()["asset_id"]
        visitor(monkeypatch)
        r = client.post(f"/parts/{aid}/for-sale", data={"for_sale": "1"},
                        follow_redirects=False)
        assert r.status_code == 303
        assert r.headers["location"].startswith("/login?next=")

    def test_the_item_page_shows_a_visitor_neither_tick_nor_marker(
...
class TestTheNamedSetRatherThanAHabit:
    ...
    def test_the_haystack_leaves_out_every_owner_only_column_for_a_visitor(
            self, db, part):
    ...
    def test_an_unflagged_item_reads_identically_for_both(self, db, part):
        ...
        assert _haystack(db, row, {}, authed=False) == _haystack(db, row, {},
                                                                 authed=True)
...
class TestItSurvivesTheRestOfTheSite:
    ...
    def test_disposing_a_flagged_item_leaves_the_flag_alone(self, client, db, part):
        ...
        aid = part()["asset_id"]
        flag(client, "parts", aid)
        client.post(f"/parts/{aid}/dispose", data={"note": "sold", "date": "2026-01-01"},
                    follow_redirects=False)
        db.expire_all()
        row = db.get(Part, aid)
    ...
    def test_the_decision_is_written_down(self):
        ...
        assert "ADR-0018" in (root / "docs" / "architecture.md").read_text(
            encoding="utf-8")
