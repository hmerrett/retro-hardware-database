...
def tasks(db: Session, project_id: str) -> list[ProjectTask]:
    ...
    return (
        ...
        .order_by(ProjectTask.done, ProjectTask.id)
        .all()
    )
...
def orders(db: Session, project_id: str) -> list[ProjectOrder]:
    ...
    rows = (
        db.query(ProjectOrder)
        .filter(ProjectOrder.project_id == project_id)
        .order_by(ProjectOrder.id)
        .all()
    )
    rows.sort(key=lambda o: (bool(o.delivered), o.expected_at is None, o.expected_at or o.id))
    return rows
...
def project_for(db: Session, asset_id: str, authed: bool = True) -> Project | None:
    ...
    q = (
        db.query(Project)
        .join(ProjectAsset, ProjectAsset.project_id == Project.asset_id)
...
def project_wide_tasks(db: Session, project_id: str) -> list[ProjectTask]:
    ...
    return (
        db.query(ProjectTask)
        .filter(ProjectTask.project_id == project_id, ProjectTask.asset_id.is_(None))
        .order_by(ProjectTask.done, ProjectTask.id)
        .all()
    )
...
def tasks_for_asset(db: Session, asset_id: str, authed: bool = True) -> list[ProjectTask]:
    ...
    q = (
        db.query(ProjectTask)
        .join(Project, Project.asset_id == ProjectTask.project_id)
...
def add_asset(
    ...
    if held is not None:
        (
            db.query(ProjectTask)
            .filter(ProjectTask.asset_id == asset_id, ProjectTask.project_id == held.project_id)
...
def summaries(db: Session, authed: bool = True) -> list[Summary]:
    ...
    out.sort(key=lambda r: (rank(r["p"].status), (r["p"].name or "").lower(), r["p"].asset_id))
    return out
...
def open_projects(db: Session) -> list[Project]:
    ...
    return (
        db.query(Project)
        .filter(Project.status.notin_(CLOSED))
