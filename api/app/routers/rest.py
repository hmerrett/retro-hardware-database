"""The REST API over the register: computers and parts.

Their pages went out into `routers/computers.py` and `routers/parts.py`; this is
the other half of the same two groups, kept together because a caller sees one
surface and because the helpers under it -- turning a body into columns, checking
a link before it is written, reading a machine's catalogue identity back out --
are shared between them. One file rather than two so those do not end up in a
third.

The published shape is pinned in `api/openapi.json` and checked by the suite
(ADR-0010): a change a caller could see fails that test, and the diff of that file
is the change being asked for.
"""
...
from ..work import _take_on_work, _work_from_api
