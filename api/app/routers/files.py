"""The files kept beside the register, and what each one is for.

Nine routes and the two helpers between them: handing the bytes over, taking an
upload in, attaching it to a model or to one unit, detaching it, tagging it,
publishing it, deleting it, the page that lists everything, and the same list over
the API.

`filesdb` owns the links and the bytes; this is the five things a person does with
a file and the two ways of reading the list. It is the first group lifted out of
main that writes to the register's history, which is why `history` came out just
before it (workflow-and-ci).
"""
...
from .. import filesdb
from ..common import _register_order, to_dict
