...
def _facts(db: Session, st: Stats, this_year: int) -> list[Fact]:
    ...
    if st["top_type"]:
        add(
            "Most collected thing",
            entry.type_label(st["top_type"][0]),
    ...
    if st["fitted_kb"]:
        add(
            "Memory fitted, everything added up",
            entry.fmt_kb(st["fitted_kb"], True),
            "across every machine here",
            "/browse?f=ramfitted",
        )
    if st["stored_kb"]:
        ...
        how = (
            f"about {share:.0%} of one modern phone"
            if share < 1
            else f"{share:.1f} modern phones' worth"
        )
        add("Storage, everything added up", _big_total(st["stored_kb"]), how, "/browse?f=storage")
    ...
    if st["chips"]:
        add(
            "Memory chips counted individually",
            str(int(st["chips"])),
            "soldered or socketed on a board",
            "/browse?f=chips",
        )
    if st["drives"]:
        add(
            "Drives fitted",
            str(int(st["drives"])),
            f"{st['gotek']} of them a Gotek"
            if st["gotek"]
            else "in the machines, counted individually",
            "/browse?f=drives",
        )
    ...
    if st["oldest_held"]:
        add(
            ...
            named(st["oldest_held"]),
            f"since {st['oldest_held'].acquired_date}",
            "/browse?f=held",
        )
    if st["fitted_per_machine"] is not None and st["fitted"]:
        add(
            "Fitted in a machine",
            str(st["fitted"]),
            f"{st['fitted_per_machine']} per machine on average",
            "/browse?f=fitted",
        )
    if st["spares"]:
        add("Spares on the shelf", str(st["spares"]), "waiting for a home", "/browse?f=spares")
    if st["disposed"]:
    ...
    for size, count in _held(
        ...
        .filter(ComputerDrive.size != "", ComputerDrive.kind.in_(("floppy", "Gotek"))),
        Computer,
    ):
        kb = entry.to_kb(size)
        if kb:
    ...
    if floppy_kb and floppy_n >= 3:
        add(
            "Every floppy at once",
            f"{round(floppy_kb / 1024)} {entry.MIB}",
    ...
    if lonely:
        ...
        add(
            ...
            if len(lonely) == 1
            else "categories with a single example: "
    ...
    if shots:
        ...
        if obj and n > 1:
            kind = "computers" if isinstance(obj, Computer) else "parts"
            add("Most photographed", named(obj), f"{n} pictures of it", f"/{kind}/{obj.asset_id}")
    if shots and st["photos"] and len(held_ids):
        add(
            "Photographs per thing",
            f"{st['photos'] / len(held_ids):.1f}",
            f"{st['photos']} pictures of {len(held_ids)} things",
            "/browse?f=photos",
        )

    maker_counts = (
        _held(
            db.query(Part.manufacturer, func.count(Part.asset_id)).filter(
                Part.manufacturer.isnot(None), Part.manufacturer != ""
            ),
            Part,
        )
    ...
    if maker_counts:
        add(
            "Names on the parts",
            str(len(maker_counts)),
            "distinct makers in the register",
            "/browse?f=parts",
        )
    singles = [m for m, n in maker_counts if n == 1]
    if len(singles) >= 3:
        add(
            "Makers represented once",
            str(len(singles)),
            f"of {len(maker_counts)}, a single part each: " + ", ".join(sorted(singles)[:3]),
            "/browse?f=parts",
        )
    ...
    arrivals = Counter(
        d.year
        for (d,) in _held(db.query(Part.acquired_date).filter(Part.acquired_date.isnot(None)), Part)
    )
    if arrivals:
        year, n = max(arrivals.items(), key=lambda kv: (kv[1], kv[0]))
        if n > 1:
            add("Busiest year for buying", str(year), f"{n} parts arrived", "/browse?f=held")

    source = (
        _held(
            db.query(Part.source, func.count(Part.asset_id)).filter(
                Part.source.isnot(None), Part.source != ""
            ),
            Part,
        )
    ...
    caps = [
        (kb, pid)
        for pid, kb in _held(
            db.query(StorageSpec.part_id, StorageSpec.capacity_kb)
    ...
    chips = (
        _held(
            db.query(ComputerRamChip.computer_id, func.sum(ComputerRamChip.count)).join(
                Computer, Computer.asset_id == ComputerRamChip.computer_id
            ),
            Computer,
        )
        .group_by(ComputerRamChip.computer_id)
        .order_by(func.sum(ComputerRamChip.count).desc())
        .first()
    )
    if chips:
    ...
    if ifaces:
        ...
        add(
            "How a drive usually attaches",
            ifaces[0][0],
            f"{ifaces[0][1]} of {total} drives",
            "/browse?f=storage",
        )
    if len(ifaces) >= 3:
        add(
            "Ways of attaching a drive",
            str(len(ifaces)),
            "in use here: "
            + ", ".join(i for i, _n in ifaces[:4])
            + ("…" if len(ifaces) > 4 else ""),
            "/browse?f=storage",
        )
    ...
    if discs and discs[0][1] > 1:
        add(
            "The usual disc",
            discs[0][0],
            f"{discs[0][1]} optical drives take it",
            "/browse?f=storage",
        )
    ...
    disk_caps = [
        kb
        for (kb,) in _held(
            db.query(StorageSpec.capacity_kb)
            .join(Part, Part.asset_id == StorageSpec.part_id)
            .filter(
                StorageSpec.kind == entry.DISK_KIND,
                StorageSpec.capacity_kb.isnot(None),
                StorageSpec.capacity_kb > 0,
            ),
            Part,
        )
    ]
    if len(disk_caps) >= 2:
        add(
            "Every hard disk added up",
            _big_total(sum(disk_caps)),
            f"across {len(disk_caps)} of them",
            "/browse?f=storage",
        )
    ...
    clamped = (
        _held(
            db.query(func.count(StorageSpec.part_id))
            .join(Part, Part.asset_id == StorageSpec.part_id)
            .filter(StorageSpec.chs_c == 16383, StorageSpec.chs_h == 16, StorageSpec.chs_s == 63),
            Part,
        ).scalar()
        or 0
    )
    if clamped:
        add(
            "Drives that lie about their shape",
            str(clamped),
    ...
    if yellowed:
        ...
        add(
            "Bezels gone yellow",
            str(n),
            f"most often {yellowed[0][0].lower()}",
            "/browse?f=storage",
        )
    ...
    if years:
        ...
        if len(decades) > 1:
            add("Best represented decade", f"{decade}s", f"{n} things made then", "/browse?f=year")
    ...
    untested = (
        _held(
            db.query(func.count(Part.asset_id)).filter(Part.condition == "Untested"), Part
        ).scalar()
        or 0
    )
    if untested and st["n_parts"]:
        add(
            "Never tested",
            str(untested),
            f"{round(100 * untested / st['n_parts'])}% of the parts, plugged into nothing yet",
            "/browse?f=condition&v=Untested",
        )
...
def _facts_boards(db: Session, st: Stats) -> list[Fact]:
    ...
    if shapes:
        ...
        if odd:
            out.append(
                _fact(
                    "Boards that fit nothing else",
                    str(odd),
                    f"of {n_shaped} with a shape recorded, built to a plan of their maker's own",
                    "/browse?f=boards",
                )
            )
        named_shapes = [(s, n) for s, n in shapes if not s.lower().startswith("proprietary")]
        if named_shapes:
            out.append(
                _fact(
    ...
    if len(chipsets) >= 3:
        out.append(
            _fact(
                "Chipsets named",
                str(len(chipsets)),
                f"across {sum(n for _c, n in chipsets)} boards — hardly any two of them agree",
                "/browse?f=boards",
            )
        )

    cache = _held(
        db.query(func.sum(MotherboardSpec.cache_kb), func.count(MotherboardSpec.part_id))
        .join(Part, Part.asset_id == MotherboardSpec.part_id)
        .filter(MotherboardSpec.cache_kb.isnot(None), MotherboardSpec.cache_kb > 0),
        Part,
    ).first()
    if cache and cache[0]:
        out.append(
            _fact(
                "Cache on the boards, added up",
    ...
    if onboard:
        out.append(
            _fact(
                "Boards with the video already on them",
                str(onboard),
    ...
    boards = {
        a
        for (a,) in _held(
            db.query(MotherboardSpec.part_id).join(Part, Part.asset_id == MotherboardSpec.part_id),
            Part,
        )
    }
    ...
    per_board = sorted(
        _held(
            db.query(PartSlot.part_id, func.sum(PartSlot.count)).join(
                Part, Part.asset_id == PartSlot.part_id
            ),
            Part,
        )
        .group_by(PartSlot.part_id)
        .all(),
        key=lambda r: -r[1],
    )
    if per_board:
    ...
    isa = (
        _held(
            db.query(func.sum(PartSlot.count))
            .join(Part, Part.asset_id == PartSlot.part_id)
            .filter(PartSlot.bus.like("%ISA%")),
            Part,
        ).scalar()
        or 0
    )
    ...
    sockets = sorted(
        _held(
            db.query(PartRamSlot.slot_type, func.sum(PartRamSlot.count)).join(
                Part, Part.asset_id == PartRamSlot.part_id
            ),
            Part,
        )
        .group_by(PartRamSlot.slot_type)
        .all(),
    ...
    if sockets:
        ...
        holders = (
            _held(
                db.query(func.count(func.distinct(PartRamSlot.part_id))).join(
                    Part, Part.asset_id == PartRamSlot.part_id
                ),
                Part,
            ).scalar()
...
def _facts_cards(db: Session, st: Stats) -> list[Fact]:
    ...
    if prevga:
        out.append(
            _fact(
                ...
                str(prevga),
                "nothing a VGA monitor would accept",
                "/browse?f=prevga",
            )
        )
    ...
    vram = [
        kb
        for (kb,) in _held(
            db.query(VideoSpec.memory_kb)
            .join(Part, Part.asset_id == VideoSpec.part_id)
            .filter(VideoSpec.memory_kb.isnot(None), VideoSpec.memory_kb > 0),
            Part,
        )
    ]
    if vram:
        out.append(
            _fact(
                "Video memory, everything added up",
        ...
        out.append(
            _fact(
                "The usual amount of video memory",
    ...
    if len(vram) >= 2 and max(vram) > min(vram):
        out.append(
            _fact(
                "Biggest and smallest video memory",
                f"{entry.fmt_kb(max(vram), True)} / {entry.fmt_kb(min(vram), True)}",
    ...
    if sound and sound[0][1] > 1:
        out.append(
            _fact(
                "The commonest sound chip",
                sound[0][0],
                f"{sound[0][1]} of the {sum(n for _c, n in sound)} cards that name one",
                "/browse?f=type&v=sound",
            )
        )
    ...
    if len(nets) >= 2:
        out.append(
            _fact(
                "Network cards, either side of the changeover",
                f"{nets[0][1]} / {nets[1][1]}",
    ...
    for model in (VideoSpec, SoundSpec, NetworkSpec, IoSpec):
        ...
        named_bus += _spec_count(db, model, model.interface.isnot(None), model.interface != "")
        on_isa += _spec_count(db, model, model.interface.in_(ISA_BUSES))
        chip_blank += _spec_count(db, model, (model.chip.is_(None)) | (model.chip == ""))
    ...
    if on_isa and named_bus:
        out.append(
            _fact(
                "Cards that never left ISA",
                str(on_isa),
                f"of {named_bus} with a bus recorded, {round(100 * on_isa / named_bus)}% of them",
                "/browse?f=cards",
            )
        )
    ...
    if chip_blank and n_cards:
        out.append(
            _fact(
                "Cards whose chip nobody has written down",
                str(chip_blank),
                f"of {n_cards}: mostly the serial and network cards, where nobody thinks to look",
                "/browse?f=nochip",
            )
        )
    ...
    ports = _held(
        db.query(func.sum(PartPort.count), func.count(func.distinct(PartPort.part_id))).join(
            Part, Part.asset_id == PartPort.part_id
        ),
        Part,
    ).first()
    if ports and ports[0]:
        ...
        out.append(
            _fact(
                ...
                + (
                    f" — the list below ranks only the commonest {len(st['ports'])}"
                    if int(ports[0]) > shown
                    else ""
                ),
                "/browse?f=ports",
            )
    ...
    per_carrier = sorted(
        _held(
            db.query(PartPort.part_id, func.sum(PartPort.count)).join(
                Part, Part.asset_id == PartPort.part_id
            ),
            Part,
        )
        .group_by(PartPort.part_id)
        .all(),
        key=lambda r: -r[1],
    )
    if per_carrier and int(per_carrier[0][1]) > 1:
        ...
        if holder:
            out.append(
                _fact(
                    "Most ports on one thing",
                    str(int(per_carrier[0][1])),
                    _named(holder),
                    f"/parts/{holder.asset_id}",
                )
            )
...
def _facts_drives(db: Session, st: Stats) -> list[Fact]:
    ...
    old = _spec_rank(db, StorageSpec, StorageSpec.interface, StorageSpec.interface.in_(GONE))
    if old:
        out.append(
            _fact(
                "Drives on an interface nobody uses",
    ...
    rpm = sorted(
        _held(
            db.query(StorageSpec.speed_rpm)
            .join(Part, Part.asset_id == StorageSpec.part_id)
            .filter(StorageSpec.speed_rpm.isnot(None), StorageSpec.speed_rpm > 0),
            Part,
        ).all()
    )
    if rpm:
    ...
    speeds = sorted(
        x
        for (x,) in _held(
            db.query(StorageSpec.speed_x)
            .join(Part, Part.asset_id == StorageSpec.part_id)
            .filter(StorageSpec.speed_x.isnot(None), StorageSpec.speed_x > 0),
            Part,
        )
    )
    if len(speeds) >= 2 and speeds[-1] > speeds[0]:
        out.append(
            _fact(
                "Fastest and slowest optical drive",
                f"{speeds[-1]}× / {speeds[0]}×",
                f"a factor of {round(speeds[-1] / speeds[0])} between them, and about ten years",
                "/browse?f=optical",
            )
        )
    ...
    # be worth summing.
    geom = _held(
        db.query(func.count(StorageSpec.part_id), func.sum(StorageSpec.chs_c))
        .join(Part, Part.asset_id == StorageSpec.part_id)
    ...
    flash = _held(
        db.query(func.sum(ComputerDrive.count))
        .join(Computer, Computer.asset_id == ComputerDrive.computer_id)
...
def _facts_machines(db: Session, st: Stats) -> list[Fact]:
    ...
    ram = Counter(
        kb
        for (kb,) in _held(
            db.query(Computer.installed_ram_kb).filter(Computer.installed_ram_kb.isnot(None)),
            Computer,
        )
    )
    ...
    if len(ram) >= 2:
        ...
        out.append(
            _fact(
                "The least memory in anything",
                entry.fmt_kb(least, True),
                f"and the most is {entry.fmt_kb(most, True)}, a factor of {round(most / least):,}",
                "/browse?f=ramfitted",
            )
        )
    ...
    os_rank = sorted(
        _held(
            db.query(Computer.os, func.count(Computer.asset_id)).filter(
                Computer.os.isnot(None), Computer.os != ""
            ),
            Computer,
        )
        .group_by(Computer.os)
        .all(),
        key=lambda r: (-r[1], r[0]),
    )
    if os_rank and n:
        out.append(
            _fact(
                "Machines with an operating system on them",
    ...
    dos = (
        _held(
            db.query(func.count(Computer.asset_id)).filter(Computer.os.like("%DOS%")), Computer
        ).scalar()
        or 0
    )
    if dos:
        out.append(
            _fact(
                "Machines running DOS of some sort",
                str(dos),
    ...
    cpus = sorted(
        _held(
            db.query(Computer.cpu, func.count(Computer.asset_id)).filter(
                Computer.cpu.isnot(None), Computer.cpu != ""
            ),
            Computer,
        )
    ...
    if cpus and cpus[0][1] > 1:
        out.append(
            _fact(
                "The commonest processor",
                cpus[0][0],
                f"{cpus[0][1]} of the {sum(x for _c, x in cpus)} machines that name one",
                "/browse?f=cpu",
            )
        )
    ...
    chassis = sorted(
        _held(
            db.query(Computer.chassis, func.count(Computer.asset_id)).filter(
                Computer.chassis.isnot(None), Computer.chassis != ""
            ),
            Computer,
        )
        .group_by(Computer.chassis)
        .all(),
        key=lambda r: (-r[1], r[0]),
    )
    if chassis:
    ...
    if portable:
        out.append(
            _fact(
                "Portable, in the period sense",
                str(portable),
                "laptops and luggables, the latter being portable only in that it had a handle",
                "/browse?f=portable",
            )
        )
    ...
    bench = (
        _held(
            db.query(func.count(Computer.asset_id)).filter(Computer.topbench.isnot(None)), Computer
        ).scalar()
        or 0
    )
    if bench and n:
...
def _facts_ages(db: Session, st: Stats, this_year: int) -> list[Fact]:
    ...
    machine_years = [
        y for (y,) in _held(db.query(Computer.year).filter(Computer.year.isnot(None)), Computer)
    ]
    part_years = [y for (y,) in _held(db.query(Part.year).filter(Part.year.isnot(None)), Part)]
    if machine_years and part_years:
        mm, mp = (
            round(sum(machine_years) / len(machine_years)),
        ...
        if mm != mp:
            ...
            out.append(
                _fact(
                    f"The {older} are the older half",
                    f"{mm} / {mp}",
    ...
    if working:
        ...
        out.append(
            _fact(
                "The oldest thing that still works",
                str(year),
                _named(obj),
                f"/{kind}/{obj.asset_id}",
            )
    ...
    if pairs:
        ...
        if before:
            out.append(
                _fact(
                    "Parts older than their machine",
                    str(before),
...
def _facts_provenance(db: Session, st: Stats) -> list[Fact]:
    ...
    nowhere = (
        _held(
            db.query(func.count(Part.asset_id)).filter(
                (Part.source == "") | (Part.source.is_(None))
            ),
            Part,
        ).scalar()
        or 0
    ...
    if nowhere and st["n_parts"]:
        out.append(
            _fact(
                "Parts with no idea where they came from",
                str(nowhere),
                f"{round(100 * nowhere / st['n_parts'])}% of them: no source recorded at all",
                "/browse?f=nosource",
            )
        )
    ...
    bought = (
        _held(db.query(func.count(Part.asset_id)).filter(Part.source.like("eBay%")), Part).scalar()
        or 0
    )
    if bought:
        out.append(
    ...
    made = (
        _held(
            db.query(func.count(Part.asset_id)).filter(Part.source.like("Self-made%")), Part
        ).scalar()
        or 0
    )
    if made:
    ...
    if arrivals and st["n_parts"]:
        ...
        if len(winners) == 1:
            out.append(
                _fact(
                    f"Things arrive on a {winners[0]}",
                    str(top),
                    "more of them than on any other day",
                    "/browse?f=held",
                )
            )
        else:
            out.append(
                _fact(
                    "The day things arrive",
                    " / ".join(winners),
                    f"tied on {top} arrivals each",
                    "/browse?f=held",
                )
            )
...
def _facts_register(db: Session, st: Stats) -> list[Fact]:
    ...
    if shots:
        ...
        if once:
            out.append(
                _fact(
                    "Things photographed exactly once",
                    str(once),
                    f"of {len(shots)} photographed at all: one angle and no more",
                    "/browse?f=photos",
                )
            )
        ...
        if many:
            out.append(
                _fact(
                    "Things photographed five times or more",
                    str(many),
    ...
    unwritten = (
        _held(
            db.query(func.count(Part.asset_id)).filter(Part.summary == "", Part.notes == ""), Part
        ).scalar()
        or 0
    )
    if unwritten and st["n_parts"]:
        out.append(
            _fact(
                "Parts nobody has written a word about",
                str(unwritten),
                f"{round(100 * unwritten / st['n_parts'])}% of them: no summary and no notes",
                "/browse?f=unwritten",
            )
        )
    linked = (
        _held(
            db.query(func.count(Part.asset_id)).filter(Part.url != "", Part.url.isnot(None)), Part
        ).scalar()
        or 0
    )
    if linked:
    ...
    images = (
        _held(
            db.query(func.count(Part.asset_id)).filter(
                Part.disk_image != "", Part.disk_image.isnot(None)
            ),
            Part,
        ).scalar()
        or 0
    ...
    dupes = sorted(
        _held(
            db.query(Part.manufacturer, Part.model, func.count(Part.asset_id)).filter(
                Part.model != "", Part.model.isnot(None)
            ),
            Part,
        )
    ...
    if repeated:
        out.append(
            _fact(
                "Things there is more than one of",
                str(len(repeated)),
        ...
        out.append(_fact("The most duplicated thing here", str(top), said, "/browse?f=dupes"))
    ...
    if short and short[0] > 1:
        out.append(
            _fact(
                "Shortest name in the register",
                f"{short[0]} characters",
                _named(short[1]),
                f"/{short[2]}/{short[1].asset_id}",
            )
        )
    return out
...
def _facts_projects(db: Session, st: Stats) -> list[Fact]:
    ...
    if open_n:
        out.append(
            _fact(
                "Projects on the go",
                str(open_n),
                f"of {total} written down: repairs, builds and machines still being looked for",
                "/projects",
            )
        )
    done = db.query(func.count(Project.asset_id)).filter(Project.status == "done").scalar() or 0
    if done:
        out.append(
            _fact(
                "Projects actually finished",
                str(done),
                f"of {total}, which is the figure a pile of half-done machines is measured against",
                "/projects",
            )
        )
    todo = db.query(func.count(ProjectTask.id)).filter(ProjectTask.done.is_(False)).scalar() or 0
    ticked = db.query(func.count(ProjectTask.id)).filter(ProjectTask.done.is_(True)).scalar() or 0
    if todo:
        out.append(
            _fact("Jobs still on the list", str(todo), f"against {ticked} ticked off", "/projects")
        )
    coming = (
        db.query(func.count(ProjectOrder.id)).filter(ProjectOrder.delivered.is_(False)).scalar()
        or 0
    )
    if coming:
        out.append(
            _fact(
                "Things in the post",
                str(coming),
                "ordered for a project and not here yet",
                "/projects",
            )
        )
    spoken_for = db.query(func.count(func.distinct(ProjectAsset.asset_id))).scalar() or 0
    if spoken_for:
        out.append(
            _fact(
                "Things spoken for by a project",
                str(spoken_for),
                "not spare, whatever the shelf says",
                "/projects",
            )
        )
    ...
    oldest = (
        db.query(Project)
        .filter(Project.status.notin_(projects.CLOSED), Project.started_at.isnot(None))
...
def _facts_condition(db: Session, st: Stats) -> list[Fact]:
    ...
    if perfect and len(rel) > len(perfect):
        out.append(
            _fact(
                "Makers with a clean sheet",
                str(len(perfect)),
                f"of {len(rel)} with {RELIABILITY_MIN}+ parts here, every one of them working",
            )
        )
    ...
    for label, key, blurb in (
        ("Broken and kept anyway", "Faulty", "known bad and still on the shelf"),
        ("Works, but not all of it", "Partially working", "the most honest category there is"),
        ("Brought back", "Restored", "working, but it did not arrive that way"),
        ("Good only for parts", "For parts/repair", "kept for what can be taken off it"),
    ):
        n = (
            _held(db.query(func.count(Part.asset_id)).filter(Part.condition == key), Part).scalar()
            or 0
        )
        if n:
    ...
    if spare_types and st["spares"]:
        out.append(
            _fact(
                "The commonest thing to have spare",
...
def _collection_stats(db: Session) -> Stats:
    ...
    makers = ranked(
        _held(db.query(Part.manufacturer, func.count(Part.asset_id)), Part).group_by(
            Part.manufacturer
        ),
        8,
    )
    types = ranked(_held(db.query(Part.type, func.count(Part.asset_id)), Part).group_by(Part.type))
    ...
    buses = ranked(
        _held(
            db.query(PartSlot.bus, func.sum(PartSlot.count)).join(
                Part, Part.asset_id == PartSlot.part_id
            ),
            Part,
        ).group_by(PartSlot.bus),
        6,
    )
    ports = ranked(
        _held(
            db.query(PartPort.port, func.sum(PartPort.count)).join(
                Part, Part.asset_id == PartPort.part_id
            ),
            Part,
        ).group_by(PartPort.port),
        6,
    )
    conditions = ranked(
        _held(db.query(Part.condition, func.count(Part.asset_id)), Part).group_by(Part.condition)
    )
    ...
    common_ram = sorted(
        ((entry.fmt_kb(kb), ram.count(kb), kb) for kb in set(ram)), key=lambda r: (-r[1], r[0])
    )
    ...
    stored_kb = (
        _held(
            db.query(func.sum(StorageSpec.capacity_kb)).join(
                Part, Part.asset_id == StorageSpec.part_id
            ),
            Part,
        ).scalar()
        or 0
    ...
    slots = (
        _held(
            db.query(func.sum(PartSlot.count)).join(Part, Part.asset_id == PartSlot.part_id), Part
        ).scalar()
        or 0
    )
    boards = (
        _held(
            db.query(func.count(func.distinct(PartSlot.part_id))).join(
                Part, Part.asset_id == PartSlot.part_id
            ),
            Part,
        ).scalar()
    ...
    chips = (
        _held(
            db.query(func.sum(ComputerRamChip.count)).join(
                Computer, Computer.asset_id == ComputerRamChip.computer_id
            ),
            Computer,
        ).scalar()
    ...
    drives = (
        _held(
            db.query(func.sum(ComputerDrive.count)).join(
                Computer, Computer.asset_id == ComputerDrive.computer_id
            ),
            Computer,
        ).scalar()
        or 0
    ...
    gotek = (
        _held(
            db.query(func.count(ComputerDrive.id))
            .join(Computer, Computer.asset_id == ComputerDrive.computer_id)
            .filter(ComputerDrive.kind == "Gotek"),
            Computer,
        ).scalar()
        or 0
    )
    working = (
        _held(
            db.query(func.count(Part.asset_id)).filter(Part.condition == "Working"), Part
        ).scalar()
        or 0
    )
    ...
    oldest_held = (
        _held(db.query(Part).filter(Part.acquired_date.isnot(None)), Part)
        .order_by(Part.acquired_date)
        .first()
    )
    ...
    return {
        ...
        "top_ram": top_ram,
        "top_ram_kb": [r[2] for r in top_ram],
